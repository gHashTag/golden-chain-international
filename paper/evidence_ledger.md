...
| E31 | A usable identity root fits the shuttle area budget | test, synthesis not silicon | The recommendation is BCH(127,57,11) in four blocks with Systematic Low Leakage Coding, reliable-bit selection keeping the positions that never voted unanimously over twenty-five enrolment reads, which is 54.4 percent of them, and a SPONGENT-based helper-data manipulation countermeasure. It occupies 3.46 of the sixteen tiles a submission may use, from 36,198 square micrometres of standard cells divided by the measured 58 percent tile utilisation - decoder 24,659, SLLC stages 3,176, countermeasure 6,215, oscillators 2,148. The oscillators are aging-resistant rather than conventional, which is a requirement and not a preference: a conventional ring oscillator bank loses 32.4 percent of its response bits over ten years and this construction tolerates 10.9 percent, leaving a ten-year margin of 3.6, while the aging-resistant design in the literature loses 7.7. The extra cost is two nMOS transistors per stage and 0.09 of a tile, estimated from the published transistor widths rather than laid out (W-INTL-154, W-INTL-160). 935 raw positions, 508 selected, 44 oscillators. The fourth block is the density headroom rule of W-INTL-228: the construction is sized against the borrowed min-entropy density derated by 1.25, the same multiple the aging figure has carried since W-INTL-196, applied here because the density turned out to be the tighter of the two - 1.13 times in hand against 1.41. It carries 228 bits of k where 200 are required, and the density reaching the extractor may fall to 0.5614 before the key stops fitting, against 0.7485 before the rule. This figure and every input behind it are recomputed from research/inputs.py by scripts/check_figures_reproduce.py in CI; the recomputation is anchored to the construction recommended now, which it was not until W-INTL-147 - for eight loops the check and this row agreed on 8.49 tiles because both were pinned to a superseded design, and a check that pins a document to a stale model reports green while the document is wrong. Movement to here: 8.79 tiles, then 5.40 by sharing the key-equation solver's multipliers, then 3.50 by re-choosing the code at the error rate reliable-bit selection had already produced, then 3.35 by sizing the SLLC encoder to its own code rather than to the one it was written for. Sixty-two percent removed in four loops, none of it by a new technique or a new source: every step revisited a decision that had been correct at the operating point where it was made. The construction carries 228 bits of k against the 140.5 the key needs at the min-entropy density that applies after selection, and the 200 it needs once the borrowed figure is derated by the headroom rule of W-INTL-228 - the second is what the construction is sized to, and the gap between them is the margin the rule buys. That after-selection density is 0.9113 rather than the measured 0.9414 - selection amplifies bias and was assumed not to, and the check that said otherwise sampled from a symmetric source model. It tolerates an entropy density down to 0.5614, and the oscillator floor is 44 oscillators, set by the pairs the raw positions need rather than by log2(R!) exceeding 128, which asks for 35. Every decoder area is verified by end-to-end decoding before it is quoted, every declared area is re-synthesised by scripts/verify_inputs.py - thirty-one of them - and research/rtl/measure_all.sh runs twenty-seven testbenches and refuses to print an area if any fails, the testbench half of it on every pull request. The whole chain runs end to end in software and an independent implementation reproduces the failure model across four orders of magnitude. Power, timing and interconnect are slack by an order of magnitude. Oscillator aging is checked and binding, and is what makes the bank aging-resistant; process corners are half closed by argument, area being corner-independent, with the slow-corner liberty named as the missing artefact; placement and routing at this utilisation remains unchecked. See research/constraint_register.md. See research/decoder_code_choice.md | an oscillator bank whose ten-year differential flip rate exceeds 10.9 percent unselected, which the literature reports at 32.4 percent for a conventional ring-oscillator bank and 7.7 for an aging-resistant one, both simulated at 90 nm - the aging-resistant arrangement is what this row now claims, and a layout showing its area cost materially above the estimated 1.86 times would move the tile figure (W-INTL-154, W-INTL-160). A low duty cycle is not a defence for the conventional arrangement: an idle ring oscillator sits at DC stress, which is the worst case for the mechanism that ages it; a measured bit error probability above fifteen percent raw, at which selection can no longer reach the effective rate the code needs; an entropy per response bit below 0.5614 after selection; or a placement and routing run that does not close at this utilisation. The application must not claim resistance to helper-data manipulation beyond what the countermeasure gives: the field states that no construction meeting practical error-correction requirements has a robustness proof. Synthesis is not silicon, and nothing here has been fabricated |
