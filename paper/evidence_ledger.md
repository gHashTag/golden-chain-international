...
| E31 | A usable identity root fits the shuttle area budget | test, synthesis not silicon | decoders measured against the SkyWater library at the typical corner on 2026-07-30, each verified in a self-checking testbench before its area was quoted and each check shown able to fail on injected faults; oscillator counts from log2(R!), the information a frequency ranking carries. Repetition concatenated with first-order Reed-Muller: 4,596 um^2 of decoder, 614 oscillators, 1.15 of the sixteen tiles a submission may use, holding to a bit error probability of 13.2 percent. The model is calibrated against the published table it derives from, five checks matching to three significant figures. See research/decoder_code_choice.md and research/code_choice_model.py | a measured bit error probability above about 13 percent, or an entropy per oscillator below the ordering bound. Note the code this project sized for five loops, BCH(255,131) at t=18, reaches the target only below 1.88 percent and is therefore not admissible - see W-INTL-57. Nothing here is fabricated; synthesis is not silicon |
