...
| E31 | A usable identity root fits the shuttle area budget | test, synthesis not silicon | decoders measured against the SkyWater library at the typical corner on 2026-07-30, each verified in a self-checking testbench before its area was quoted and each check shown able to fail on injected faults; oscillator counts bracketed by arrangement rather than taken from a bound. Repetition concatenated with first-order Reed-Muller: 4,596 um^2 of decoder, holding to a bit error probability of 13.2 percent, and between 0.65 and 14.25 of the sixteen tiles a submission may use - the low end with oscillators reused across pairs, the high end with two per response bit, which is the arrangement the one measured entropy figure was taken from. The model is calibrated against the published table it derives from, five checks matching to three significant figures. See research/decoder_code_choice.md and research/code_choice_model.py | a measured bit error probability above about 13 percent, or an entropy per oscillator below 0.471 bits, the figure measured on silicon by Wilde, Hiller and Pehl. The bracket spans a factor of twenty-two and its upper end has almost no margin; see W-INTL-59 through W-INTL-61, the last of which notes the response bits are biased, so no security level should be quoted from an average entropy figure. Note the code this project sized for five loops, BCH(255,131) at t=18, reaches the target only below 1.88 percent and is therefore not admissible - see W-INTL-57. Nothing here is fabricated; synthesis is not silicon |
