...
| E31 | A usable identity root fits the shuttle area budget | test, synthesis not silicon | BCH(127,15,27) over GF(2^7), the one code among those named in the literature that clears helper-data leakage, error tolerance and area together. Decoder measured against the SkyWater library at the typical corner on 2026-07-30: syndrome bank and Chien search 27,590 um^2, key-equation solver 73,119 um^2, total 100,709. The solver was verified in the field before its area was quoted, with the previous field re-run as a regression and an injected fault failing 77 of 82 patterns. Nineteen blocks for a 128-bit key, 2,413 raw response bits, tolerating a bit error probability up to 7.06 percent. Between 5.98 and 12.62 of the sixteen tiles a submission may use, the spread being the oscillator arrangement. See research/decoder_code_choice.md and research/code_choice_model.py | a measured bit error probability above 7 percent, an entropy per response bit below 0.9414, or a bias severe enough to need a debiasing stage - which is not in this budget and which W-INTL-65 argues the reported ring-oscillator bias range would require. Two earlier recommendations for this row were withdrawn, one for missing the error target and one for missing the leakage bound; see W-INTL-57 and W-INTL-63. Nothing here is fabricated; synthesis is not silicon |
