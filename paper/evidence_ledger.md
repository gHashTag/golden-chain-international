...
| E12 | Quantisation-aware training pipeline | test, unusually well controlled, but for 8-bit rather than 1.58-bit | research/QAT_ABLATION_RESULTS.json in gHashTag/trinity-fpga, dated 2026-07-18: 29.4M parameters, 2000 steps, three seeds with the median reported, disjoint train and validation shards, a stated significance threshold of 0.005 BPB, and an honest-limits block naming four constraints including that an earlier seed collapse was traced to data leakage and fixed. The result runs against the project's own format: FP8 E4M3 is indistinguishable from FP32 while the GoldenFloat 8-bit arm degrades by roughly forty-four times the threshold | the pipeline failing to reproduce those medians. Note the mismatch: this artefact covers 8-bit formats. The 1.58-bit claim made elsewhere needs its own run and does not inherit this one |
