...
| **Environmental variation, chiefly temperature** | open, and it was not a row here until W-INTL-235. Every error-rate figure in this work is at one temperature. The budget carries a noise term and an aging term and nothing environmental, and the only mention anywhere was a clause in the aging row asserting that ring oscillator PUFs are resilient to temperature - which is true relative to other PUFs and does not make the term zero. Both oscillators of a pair shift together, but pairs whose frequencies are close reverse, and those are exactly what reliable-bit selection is supposed to have discarded, so the residual falls where nothing budgets it. A configurable RO-PUF over 25 to 70 degrees reports intra-chip variation averaging 2.63 percent and reaching 11 on the worst chip. The recommendation carries up to 8.1 percent at no cost and does not carry 11, and a one-in-a-million word-failure target is a claim about every part rather than about the average one. Two answers exist and neither is adopted: selecting harder costs 0.25 of a tile with the fraction free (W-INTL-236, research/budget_audit.py), and enrolling at a second temperature costs no area at all - it takes the worst chip from 0.024 to 0.006 at the declared fraction, because ranking on the worse of two conditions selects the positions stable across both (W-INTL-237, research/multi_condition_enrolment.py). The second is the only lever in this work that attacks the environmental residual directly rather than absorbing it afterwards. Not adopted because the borrowed 11 percent is from a different circuit over a narrower span, and the same temperature sweep settles which answer is needed | the configurable-RO literature; nothing in this project |
...
recommended construction meets at 342. And the word failure rate, which sets the correction strength
...
| min-entropy density | number of blocks; k required; the oscillator floor; whether debiasing is needed | yes - and this loop it moved again, because the density that applies is the one *after* selection, which is 0.5889 rather than 0.7162 - and the 0.7162 is itself W-INTL-231, the published figure being a Shannon entropy where a fuzzy extractor needs a min-entropy |
...
per stage, 1.86 times the area by transistor width, 0.12 of a tile, taking the design from 3.39 to
3.51 of sixteen. The factor is an estimate from the published transistor sizes and not a layout.
...
## Independent rollback anchor, 2026-09-06

W-INTL-273 adds a separate monotone generation witness to the two-slot checkpoint model. The journal may contain two valid older records after a rollback; recovery accepts only a journal whose generation equals the independently stored anchor. In 64 deterministic trials, matching newer state was accepted 64/64, both-slot rollback was rejected 64/64, anchor tampering was rejected 64/64, stale or equal updates were rejected 128/128, canonical anchors round-tripped 64/64, malformed anchors were rejected 64/64, and successful recovery preserved state 64/64.

This is [measured] finite software and [proved] only for the declared byte layout and transition rules. The anchor is not device-backed and supplies no evidence about crash atomicity, filesystem or power-loss ordering, write barriers, rollback of the anchor itself, deployment security, hardware, or G16. Literature boundaries are recorded in `research/lit_notes_2026-09-06.md`; the sources discuss trusted counters, crash consistency, and dual-write recovery rather than this repository's model.
