...
| 2 | Word failure rate at or below one in a million | the application's own requirement | **binding only if every error must be corrected** - reliable-bit selection reaches fifteen percent raw with 974 response bits by indexing the reliable third, which no construction evaluated here does. See W-INTL-115 |
